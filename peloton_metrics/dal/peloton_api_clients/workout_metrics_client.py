...
class WorkoutMetricsClient(PelotonApiClient):
    ...
    def fetch_workout_performance_metrics(self, workout_id: str):
        performance_metrics = self.request(self.workout_performance_metrics_endpoint(workout_id)).json()
    ...
    def save_all_workouts(self, user_id: str, workout_data: list):
        ...
        last_saved_workout_timestamp = user_workout_dao.fetch_most_recently_saved_workout_timestamp(user_id)

        metrics_extractor = UserWorkoutMetricsExtractor()
        new_workouts_to_save = []
        for workout in workout_data:
            if self.is_workout_eligible_to_be_saved(workout, last_saved_workout_timestamp):
                new_workouts_to_save.append(
                    self.enrich_workout_with_performance_metrics(
                        metrics_extractor.extract_one(
                            workout,
                            user_name=user_name
                        )
                    )
    ...
    def enrich_workout_with_performance_metrics(self, workout: dict) -> dict:
        ...
        extracted_performance_data = WorkoutPerformanceExtractor.extract_all_performance_metrics(performance_data)
        return {
            **workout,
    ...
    @staticmethod
    def is_workout_eligible_to_be_saved(workout: dict, last_saved_workout_timestamp: int) -> bool:
        # Only save completed workouts that we haven't already saved.
        return workout["created_at"] > last_saved_workout_timestamp and workout['status'] == 'COMPLETE'
