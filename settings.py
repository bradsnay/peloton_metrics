...
def local() -> Settings:
    ...
    def inject_config(binder):
        binder.bind(Engine, postgresDBEngine)

    inject.configure_once(inject_config)

    return Settings(postgresDBEngine=postgresDBEngine)
...
def get_settings() -> Settings:
    ...
    if current_environment is None or current_environment not in AVAILABLE_ENVIRONMENTS:
        raise ValueError(
            f"Unsupported Environment '{current_environment}'. Available environments are: {Settings.AVAILABLE_ENVIRONMENTS}"
        )
